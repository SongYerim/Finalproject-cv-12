...
class _RouteTrackingMapScreenState extends State<RouteTrackingMapScreen> {
  ...
  void initState() {
    ...
    _initPorcupine().catchError((e, stackTrace) {
      developer.log('❌ [5.dart] _initPorcupine() 에러: $e', name: 'Porcupine', error: e, stackTrace: stackTrace);
    });
  }
  ...
  Future<void> _initPorcupine() async {
    try {
      ...
      _porcupineService.onKeywordDetected = (keyword) {
        developer.log('📞 [5.dart] onKeywordDetected 콜백 호출됨: $keyword', name: 'Porcupine');
        if (keyword == '시대야' && mounted) {
          developer.log('🎤 [5.dart] "시대야" 키워드 감지됨 - VLM 호출 시작', name: 'Porcupine');
          _captureAndUploadVLM(context);
        } else {
          developer.log('⚠️ [5.dart] 키워드 불일치 또는 화면이 마운트되지 않음: keyword=$keyword, mounted=$mounted', name: 'Porcupine');
        }
      };
      developer.log('✅ [5.dart] onKeywordDetected 콜백 등록 완료', name: 'Porcupine');

      developer.log('🔧 [5.dart] PorcupineService.initialize() 호출', name: 'Porcupine');
      final initialized = await _porcupineService.initialize();
      
      if (initialized) {
        developer.log('✅ [5.dart] Porcupine 초기화 성공, start() 호출', name: 'Porcupine');
        final started = await _porcupineService.start();
        if (started) {
          developer.log('✅ [5.dart] Porcupine 시작 완료 - 마이크 활성화됨', name: 'Porcupine');
        } else {
      ...
      developer.log('❌ [5.dart] _initPorcupine() 예외 발생: $e', name: 'Porcupine', error: e, stackTrace: stackTrace);
    }
  }
  ...
  Future<void> _captureAndUploadVLM(BuildContext context) async {
    try {
      ...
      try {
        ...
        developer.log('❌ [5.dart] Porcupine 재시작 실패: $e', name: 'Porcupine', error: e, stackTrace: stackTrace);
      }
